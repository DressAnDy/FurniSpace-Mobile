...
import {
  DESIGNER_NOTIFICATIONS_PAGE_SIZE,
...
import { useDesignerAssignedProjectsQuery } from "../../designer/hooks/useDesignerDashboard";
import { DesignerBottomNav, DesignerFrame } from "../../designer/components/DesignerShared";
...
const DESIGNER_FILTERS: Array<{ key: SaleNotificationFilter; label: string }> = [
  { key: "all", label: "All" },
...
export function DesignerNotificationsScreen(): React.JSX.Element {
  ...
  const projectsQuery = useDesignerAssignedProjectsQuery({ page: 1, limit: 100 });
  const notificationsQuery = useSaleNotificationsQuery(filter, page, DESIGNER_NOTIFICATIONS_PAGE_SIZE);
  ...
  const totalPages = Math.max(1, Math.ceil(totalItems / DESIGNER_NOTIFICATIONS_PAGE_SIZE));
  ...
  const handleNotificationPress = (item: NotificationListItem) => {
    ...
    void navigateFromNotification(navigation, item, {
      setActiveProjectId,
      role: userRole === "DESIGNER" ? "DESIGNER" : userRole,
    })
      .catch((error: unknown) => {
      ...
      <View style={styles.screen}>
        <View style={[styles.header, { paddingTop: Math.max(insets.top, 18) + 8 }]}>
          <View style={styles.headerTopRow}>
            <Pressable style={saleS.headerIcon} onPress={() => navigation.navigate("DesignerDashboard")}>
              <AppIcon definition={arrowLeftIconDefinition} size={15} color={SALE.white} />
            </Pressable>
            <View style={styles.headerCopy}>
              <Text style={styles.headerEyebrow}>Designer inbox</Text>
        ...
        >
          {notificationsQuery.isPending && !notificationsQuery.data ? (
            ...
            <View style={styles.centerState}>
              <Text style={styles.emptyText}>
                {filter === "unread" ? "No unread notifications." : "No designer notifications yet."}
              </Text>
            </View>
          ) : (
              ...
              <View style={styles.swipeArea}>
                <View style={styles.listMetaRow}>
                  <Text style={styles.listMetaText}>
                    Showing {(page - 1) * DESIGNER_NOTIFICATIONS_PAGE_SIZE + 1}–
                    {Math.min(page * DESIGNER_NOTIFICATIONS_PAGE_SIZE, totalItems)} of {totalItems}
                  </Text>
                </View>

                {visibleItems.map((item) => (
                  <DesignerNotificationCard
                    key={item.id}
...
function DesignerNotificationCard({
  item,
  isNavigating,
  onPress,
}: {
